<!-- SCOPE: colcon workspace setup, ament_cmake/ament_python, package.xml, overlays, symlink installs, rosdep.
     NOT HERE: node code (→ nodes-executors.md), launch files (→ launch-system.md), CI/Docker (→ deployment.md) -->
...
_Metrics to watch:_ Look at the `T_lat` (latency) and `jitter` columns. If your $99.99^{th}$ percentile latency is high, your network stack or CPU governor needs tuning.
...
_Analysis:_ Use the `tracetools_analysis` Jupyter notebooks to generate flame graphs and callback duration histograms.
